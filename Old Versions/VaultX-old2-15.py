...
class MainApp(Frame):
    def __init__(self, parent, bag):
        ...
        self.parent = parent
        self.winfo_toplevel().title('VaultX 0.4.2')
        self.option = IntVar()
        self.bag = bag
        self.subframes = []
    ...
    def main_gate(self):
        ...
        unlock_btn.pack()
        text_widget = Label(lock_window, text=self.verbose)
        text_widget.pack()
        self.verbose('w', self.update_text)
        lock_window.pack()
        lock_window.mainloop()

    def update_text(self, message):
        self.verbose.set(message)
    ...
    def unlock(self, lock_window, key_entry):
        ...
        try:
            ...
            self.update_text('Incorrect password, or missing file.')
            return
        lock_window.destroy()
